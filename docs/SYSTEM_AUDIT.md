...
## 11) IA Refactor baseline (2026-04)

- `/`는 마케팅 랜딩이 아닌 `/admin` 진입으로 고정되었습니다.
- Admin 사이드바는 `본부 -> 지점 -> 세션 -> 라이브 콘솔` 계층을 우선으로 정렬되었습니다.
- Admin Top Bar는 범위 요약 + 안전한 빠른 액션(새로고침/고객 화면/나가기)만 유지합니다.
- Customer PWA는 `전체/테이블/콘텐츠/내 정보` 탭 구조로 단순화되었습니다.
- 중복 안내/중복 상태 노출을 줄이고 단계당 핵심 메시지 우선 원칙을 적용했습니다.
