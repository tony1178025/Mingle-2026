...
export async function POST(
  ...
  try {
    ...
    console.error("[api/admin/table-picks/open]", error);
    const message = error instanceof Error ? error.message : "테이블 픽 오픈에 실패했습니다.";
    return jsonError(message, 400, { code: "ADMIN_TABLE_PICK_OPEN_FAILED" });
  }
}
